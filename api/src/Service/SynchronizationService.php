...
class SynchronizationService
{
    ...
    {
        var_dump('synchronisationPushHandler triggered');
        $this->configuration = $configuration;
        $this->data = $data;
        if ($this->session->get('io')) {
            $this->io = $this->session->get('io');
        ...
        if (!($entity instanceof Entity)) {
            var_dump('returns cuz no entity');
            return $this->data;
        }

        var_dump('we have object entities', $entity->getObjectEntities() !== null);
    ...
    private function getObjectsFromSource(Gateway $gateway): array
    {
        ...
        $results = $this->fetchObjectsFromSource($gateway, $callServiceConfig);
     ...
     * @return array
     */
    private function fetchObjectsFromSource(Gateway $source, array $callServiceConfig, int $page = 1): array
    {
        // Get a single page
        ...
        $pageResult = $this->callService->decodeResponse($source, $response);
        ...
        if (array_key_exists('limit', $this->configuration['apiSource']) && count($results) >= $this->configuration['apiSource']['limit']) {
            $results = array_merge($results, $this->fetchObjectsFromSource($source, $callServiceConfig, $page + 1));
    ...
    /**
     * Code for testing DONT COMMIT
     */
    private function unsetNullables($object)
    {
        foreach ($object as $key => $property) {
            if (is_array($property)) {
                $object[$key] = $this->unsetNullables($property);
    ...
    private function syncToSource(Synchronization $synchronization, bool $existsInSource): Synchronization
    {
        ...
        // Code for testing
        $objectArray = $this->unsetNullables($objectArray);

        $objectString = $this->getObjectString($objectArray);

        var_dump('syncToSource triggered');
        ...
        //            );
        var_dump('OBJECT STRING: ' . $objectString);
        // die;
        ...
        //     //            ]);
        // }
        $body = $this->callService->decodeResponse($result, $synchronization->getGateway());
        var_dump('BODY: ', $body);
        die;
